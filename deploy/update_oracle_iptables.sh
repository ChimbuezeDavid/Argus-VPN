...
echo "=== Updating Multi-City Routing Rules on Oracle Gateway ==="

# Flush and recreate all city chains
...
# Clean previous PREROUTING rules on wg0
sudo iptables -t nat -D PREROUTING -i wg0 -s 10.8.1.0/24 -p tcp -j REDSOCKS_NY 2>/dev/null || true
sudo iptables -t nat -D PREROUTING -i wg0 -s 10.8.2.0/24 -p tcp -j REDSOCKS_LA 2>/dev/null || true
sudo iptables -t nat -D PREROUTING -i wg0 -s 10.8.3.0/24 -p tcp -j REDSOCKS_UK 2>/dev/null || true
sudo iptables -t nat -D PREROUTING -i wg0 -s 10.8.4.0/24 -p tcp -j REDSOCKS_ES 2>/dev/null || true
sudo iptables -t nat -D PREROUTING -i wg0 -s 10.8.5.0/24 -p tcp -j REDSOCKS_JP 2>/dev/null || true
sudo iptables -t nat -D PREROUTING -i wg0 -s 10.8.0.0/24 -p tcp -j REDSOCKS_NY 2>/dev/null || true

# WireGuard Client Subnet Redirections:
...
echo "=== Multi-City IPTables Rules Successfully Configured! ==="
